resource "aws_instance" "aws14_bastion" {
    ami = data.aws_ami.ubuntu.image_id
    instance_type = "t2.micro"
    key_name = "aws14-key"
    # 보안 그룹
    vpc_security_group_ids = [aws_security_group.aws14_ssh_sg.id]
    # 서브넷
    subnet_id = data.terraform_remote_state.aws14_vpc.outputs.public_subnet2a
    # 가용 영역
